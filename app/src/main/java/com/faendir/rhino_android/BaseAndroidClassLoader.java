...
abstract class BaseAndroidClassLoader extends ClassLoader implements GeneratedClassLoader {
    ...
    public Class<?> defineClass(String name, byte[] data) {
        try {
            ...
            classFile.getMagic();
            dexFile.add(CfTranslator.translate(dxContext, classFile, null, new CfOptions(), dexOptions, dexFile));
            Dex dex = new Dex(dexFile.toDex(null, false));
            Dex oldDex = getLastDex();
            if (oldDex != null) {
                dex = new DexMerger(new Dex[]{dex, oldDex}, CollisionPolicy.KEEP_FIRST, dxContext).merge();
            }
